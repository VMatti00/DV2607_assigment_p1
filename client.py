...
def launch_attack():  # Implement this
    ...
    # <START ATTACK CODE>
    # Implement the launch_attack function in the client.py file. 
    # Onetip is to use the submit_data function to send data to the server 
    # API. Then use your own function to attack the server with that password 
    # very inaccurate data that you reach a ROC-AUC of 50%, or lower, compared to 
    # baseline. Record and compare the results from the original baseline report.

    # The data is stored in the train_data variable, which is a pandas
    # dataframe. The dataframe has the same structure as the original data
    # file, but with the first column being the UID. The UID is a unique
    # identifier for each data entry, and is used by the server to keep track
    # of which data entries are new and which are old. The UID must be
    # included in the data sent to the server, but it is not used for
    # training the model. The class attribute 'default payment next month' is
    # the target attribute, and must be either 0 or 1. The other attributes
    # are the features used for training the model. The features are all
    # integers, except for the AGE attribute which is a float. The AGE
    # attribute must be converted to an integer before sending it to the
    # server. The server will not accept any data that does not conform to
    # these requirements.


    # we begin the poisoning attack by implementing adam algorithm
    # we will use the same code as in the server.py file
    # for each elemrnt of q [1, m] do
    # for i = 1 to T do
    #   x_i = x_i - eta * (1/sqrt(v_i) + epsilon) * m_i
    #   v_i = v_i + m_i^2
    #   m_i = beta_1 * m_i + (1 - beta_1) * g_i
    #   g_i = (1 - beta_2) * g_i + beta_2 * (g_i)^2
    #   m_hat = m_i / (1 - beta_1^i)
    #   v_hat = v_i / (1 - beta_2^i)
    #   x_i = x_i - eta * m_hat / (sqrt(v_hat) + epsilon)
    # end for
    # end for
    # Now we will implement the code in python
    # v_i = 0
    # m_i = 0
    # g_i = 0
    # # train_data["default payment next month"] can only asume the values 0 or 1
    # for i in range(1, 10000):
    #     for j in range(1, 2):
    #         x_i = train_data["LIMIT_BAL"] - 0.1 * (1 / (j ** 0.5 + 0.00000001)) * (i ** 2)
    #         t_i = train_data["default payment next month"] - 0.1 * (1 / (j ** 0.5 + 0.00000001)) * (i ** 2)
    #         v_i = v_i + (i ** 2) 
    #         m_i = 0.9 * m_i + (1 - 0.9) * (i ** 2)
    #         g_i = (1 - 0.999) * g_i + 0.999 * (i ** 2)
    #         m_hat = m_i / (1 - 0.9 ** i)
    #         v_hat = v_i / (1 - 0.999 ** i)
    #         x_i[j] = x_i[j] - 0.1 * m_hat / ((v_hat ** 0.5) + 0.00000001)
    #         t_i[j] = t_i[j] - 0.1 * m_hat / ((v_hat ** 0.5) + 0.00000001)
    ...
    # train_data["LIMIT_BAL"] = x_i
    # # train_data["default payment next month"] = t_i
    # # Launch the attack
    ...
    for i in range(0, 5000):
        for j in range(1, 2):
            ...
            PAY_0 = train_data["PAY_0"][i] - 0.1 * (1 / (j ** 0.5 + 0.00000001)) * (i ** 2)
            PAY_2 = train_data["PAY_2"][i] - 0.1 * (1 / (j ** 0.5 + 0.00000001)) * (i ** 2)
            PAY_3 = train_data["PAY_3"][i] - 0.1 * (1 / (j ** 0.5 + 0.00000001)) * (i ** 2)
            PAY_4 = train_data["PAY_4"][i] - 0.1 * (1 / (j ** 0.5 + 0.00000001)) * (i ** 2)
            PAY_5 = train_data["PAY_5"][i] - 0.1 * (1 / (j ** 0.5 + 0.00000001)) * (i ** 2)
            PAY_6 = train_data["PAY_6"][i] - 0.1 * (1 / (j ** 0.5 + 0.00000001)) * (i ** 2)
